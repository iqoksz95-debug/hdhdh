...
local function xklSetKeybind(input)
    ...
    xklKeybindConnection = UserInputService.InputBegan:Connect(function(input, gameProcessed)
...
local function updateESPs()
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        local isTeamMate = _G.ayl_TeamCheck and LocalPlayer.Team == player.Team
...
local function alpMain(plr)
    ...
    local function alpUpdater()
        ...
        if plr.Character ~= nil and plr.Character:FindFirstChild("Humanoid") ~= nil and plr.Character:FindFirstChild("HumanoidRootPart") ~= nil and plr.Character.Humanoid.Health > 0 and plr.Character:FindFirstChild("Head") ~= nil then
            ...
            local alpHumPos, vis = Camera:WorldToViewportPoint(alpHum.HumanoidRootPart.Position)
            if vis then
                ...
                alporipart.CFrame = CFrame.new(alpHum.HumanoidRootPart.CFrame.Position, Camera.CFrame.Position)
                local alpSizeX = alporipart.Size.X
                local alpSizeY = alporipart.Size.Y
                local alpTL = Camera:WorldToViewportPoint((alporipart.CFrame * CFrame.new(alpSizeX, alpSizeY, 0)).p)
                local alpTR = Camera:WorldToViewportPoint((alporipart.CFrame * CFrame.new(-alpSizeX, alpSizeY, 0)).p)
                local alpBL = Camera:WorldToViewportPoint((alporipart.CFrame * CFrame.new(alpSizeX, -alpSizeY, 0)).p)
                local alpBR = Camera:WorldToViewportPoint((alporipart.CFrame * CFrame.new(-alpSizeX, -alpSizeY, 0)).p)
                ...
                local alpratio = (Camera.CFrame.p - alpHum.HumanoidRootPart.Position).magnitude
                ...
                if alpboxSettings.alpAutothickness then
                    local distance = (LocalPlayer.Character.HumanoidRootPart.Position - alporipart.Position).magnitude
...
local function setXjxFrozen(state)
    local xjxCharacter = LocalPlayer.Character
...
task.spawn(function()
	while true do
		...
		TimeTag:SetColor(Color3.fromHex("#733dd1"))
...
local AimSection = TabHandles.Aim:Section({
    Title = "AimBot",
    Icon = "bird",
})

local featureToggle = AimSection:Toggle({
    Title = "AimBot",
    Value = false,
    Callback = function(state)
...
local HitBoxSection = TabHandles.Aim:Section({
    Title = "HitBox",
    Icon = "bird",
})

local featureToggle = HitBoxSection:Toggle({
    Title = "HitBox",
    Value = false,
    Callback = function(state) 
...
local transparencySlider = HitBoxSection:Slider({
...
local transparencySlider = HitBoxSection:Slider({
    Title = "Limb Size",
    Value = { Min = 5, Max = 500, Default = le.LIMB_SIZE },
    Step = 0.1,
    Callback = function(value)
        le.LIMB_SIZE = value
    end
...
local featureToggle = EspSection:Toggle({
    Title = "Players Esp",
    Value = false,
    Callback = function(state)
        toggleESP(state)
...
local OtherSection = TabHandles.Esp:Section({
...
local featureToggle = OtherSection:Toggle({
    Title = "Brightness",
    Value = false,
    Callback = function(state)
        if state then
...
local PlayerSection = TabHandles.Player:Section({
    Title = "Player Functions",
    Icon = "bird",
})

local featureToggle = PlayerSection:Toggle({
    Title = "Freez",
    Value = false,
    Callback = function(state)
        setXjxFrozen(state)
...
local ScriptSection = TabHandles.Scripts:Section({
    Title = "Scripts",
    Icon = "bird",
})
